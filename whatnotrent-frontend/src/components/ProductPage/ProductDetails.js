    ...
    <>
      <div className={"container row m-auto justify-content-around"}>
        <SlideShow
          ...
          photos={productDetails.product["photos"]}
          classNames={"col-sm-12 col-lg-7 col-xl-5 mt-5"}
        />
        <ProductInfo
          product={productDetails.product}
          className={"mt-lg-5 col-lg-5 col-xl-7 text-center"}
        />
      </div>
    </>
  );
...
async function populateProductDetails(productId) {
  ...
  const response = await axios.get(ApiRoutes.Product(productId), authHeader());
